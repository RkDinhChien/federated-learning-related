...
class GradientDefenseModule:
    ...
    def _compute_anomaly_score(self, gradient: torch.Tensor) -> float:
        ...
        # Handle variable batch sizes by flattening
        grad_flat = gradient.flatten()
        ema_flat = self.grad_ema.flatten()
        
        # If sizes don't match, resize EMA to match current gradient
        if grad_flat.shape[0] != ema_flat.shape[0]:
            # Use average per element for comparison
            grad_norm = grad_flat.norm().item()
            ema_norm = ema_flat.norm().item()
            # Normalize by gradient size
            numerator = abs(grad_norm - ema_norm)
            denominator = ema_norm + 1e-8
        else:
            grad_diff = grad_flat - ema_flat
            numerator = grad_diff.norm().item()
            denominator = ema_flat.norm().item() + 1e-8
